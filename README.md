...
一个轻量、从头实现的 Transformer 教学/实验仓库（带一些实用改进）。适合理解 Transformer 核心部件、玩玩有/无 teacher forcing、scheduled sampling、以及观察注意力权重的行为。
...
patience = 5
wait = 0
min_delta = 0.001 # or some small value
...
**`greedy_decode`**：用于验证时的贪心解码。**注意**它返回的预测序列长度可能因 EOS 出现位置不同而变化（不包含初始 SOS）。如果你要把所有 batch 预测拼接为一个张量以做批量统计，请在拼接前做 padding 或按最大长度统一截断/补齐，否则 `torch.cat` 会因尺寸不一致报错（见常见错误）。
...
如果你把仓库发到 GitHub 并希望得到帮助，可以提交一个 Issue 来讨论 README 的特定细节（例如加上 badge、示例图、attention 可视化脚本、CI 配置等）。
